...
async function indexRecord(record, commitNow = false) {
  console.log('db: indexing record')

  return await recordsIndex.addDocuments([sanitizeRecord(record)]);
  if(record) {
...
async function indexPendingRecord(record, commitNow = false) {
  // console.log('db: indexing pending record')

  return await pendingIndex.addDocuments([sanitizeRecord(record)]);
...
async function waitForTasks(params, waitCycleCount = 3) { // blocks until all matching tasks are completed
  ...
  while(true) {
    ...
    if(tasks.total == 0 && noTaskCycles++ >= waitCycleCount) break;
    if(tasks.total > 0) noTaskCycles = 0;
