...
async function maybeIndexRecord(record) {
	console.log('maybeIndexRecord', recordToString(record));
	if(record.isDeleted) {
		console.log('deleted, not indexing')
		return Promise.resolve(false);
	}

	let uri = getRecordUri(record);
	// console.log(uri)
	// console.log(indexedRecordsCache)
	if(indexedRecordsCache.has(uri)){
		console.log('in cache, already indexed')
		return false;
	}
	indexedRecordsCache.add(uri);

	let localRecord = await getRecord(record);

	// console.log(localRecord)
	if(localRecord && !localRecord.isDeleted && !isRecordNewer(record, localRecord)) {
		console.log('not newer, not indexing')
		return false;
	}
	console.log('indexing')
	return indexRecord(record, true).then(() => true);
}
...
function maybeFetchAndIndexPendingRecordUri(uri) {
	console.log('maybeFetchAndIndexPendingRecordUri', uri)
	uri = parseUri(uri);
