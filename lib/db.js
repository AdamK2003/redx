...
async function getFilteredRecords(query, size = 100, from = 0, index = recordsIndex) {
  ...
  const removeArgs = ['q', 'matchingStrategy', 'attributesToHighlight', 'attributesToCrop', 'cropLength', 'attributesToRetrieve'];
  ...
  query.limit = size;
  if(!query.limit) delete query.limit;
  if(query.limit == Infinity) delete query.limit;
  query.offset = from;
  ...
  let queryRes = await index.getDocuments(query) // the search query is in the options object
  ...
  if(size > maxQuerySize && hits.length >= maxQuerySize) {
    hits = [...hits, ...(await getFilteredRecords(query, size - maxQuerySize, from + maxQuerySize, index)).hits];
  }
