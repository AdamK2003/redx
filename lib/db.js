...
async function waitForTasks(params) { // blocks until all matching tasks are completed
  ...
  if(params.indexes) {
    for(let index of params.indexes) {
      if(indexes[index]) indexesFilter.push(indexes[index].uid);
    }
  }

  let statuses = params.statuses || ['pending', 'processing'];
  let types = params.types || ['documentAdditionOrUpdate', 'documentDeletion']
  indexesFilter = indexesFilter || [pendingIndex.uid, recordsIndex.uid];
